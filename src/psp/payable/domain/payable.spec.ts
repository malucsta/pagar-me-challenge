...
describe('Payable Domain Entity', () => {
  ...
  describe('Payable creation', () => {
    it('should return right with created payable if no id is provided', () => {
      ...
      expect(payable.client.value).toBe('913c5e59-53aa-47d7-8753-66aa49584e89');
      expect(payable.transaction.value).toBe(
  ...
  describe('Construct valid payable method', () => {
    it('should return a valid payable', () => {
      ...
      expect(values.client.value).toBe(valuesToUpdate.client);
      expect(values.transaction.value).toBe(valuesToUpdate.transaction);
    });
  });
});

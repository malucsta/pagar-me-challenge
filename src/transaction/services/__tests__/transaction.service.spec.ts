...
import { TransactionNotFoundError } from '../../../transaction/domain/errors/not-found';
import {
  clientFoundById,
...
import { InvalidArgumentError } from '../../../transaction/domain/errors/invalid-argument';
...
describe('Transaction Service', () => {
  ...
  describe('FindAll', () => {
    ...
    it('should return left with TransactionNotFoundError if no transaction was found', async () => {
      ...
      expect(transactions.value).toBeInstanceOf(TransactionNotFoundError);
    });
  });
  ...
  describe('FindById', () => {
    ...
    it('should return left TransactionNotFoundError if transaction does not exist', async () => {
      ...
      expect(transaction.isLeft()).toBe(true);
      expect(transaction.value).toBeInstanceOf(TransactionNotFoundError);
    });
  });
  ...
  describe('Create', () => {
    ...
    it('should return left with ClientNotFoundError if client does not exist', async () => {
      jest
        ...
        .mockResolvedValue(
          left(new ClientNotFoundError(createTransactionDTO.client)),
        );
      const createdTransaction = await transactionService.create(
        createTransactionDTO,
      );
      ...
      expect(repository.save).toBeCalledTimes(0);
      expect(createdTransaction.isLeft()).toBe(true);
      expect(createdTransaction.value).toBeInstanceOf(ClientNotFoundError);
    });
  });
});
